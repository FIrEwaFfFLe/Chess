...
import GudBot
from Bad_Bots import oppositeOfKindaGudBot
from Bad_Bots import equalSides
from Bad_Bots import XORdiff
from Bad_Bots import XORmin
from Bad_Bots import priorities
from Bad_Bots import onesquare
from Bad_Bots import maxsquare
from Bad_Bots import godsavethequeen
from Bad_Bots import randomm
import game
...
if (not ai) or play == 0:
    for y in range(8):
        for x in range(8):
            if game.board[y][x] != 0:
                pie.append(SP("Chess-main/sprites/" + qwerty[game.board[y][x]] + ".png", x * u, y * u))
else:
    for y in range(8):
        for x in range(8):
            if game.board[y][x] != 0:
                pie.append(SP("Chess-main/sprites/" + qwerty[game.board[y][x]] + ".png", (7 - x) * u, (7 - y) * u))
...
def main():
    ...
    if turn == play and ai != 0 and game.event <= 1:
        move_list = []
        qwe = randomm.main(game.board, game.last_first_move, game.castle, game.kings, turn)
        game.move(qwe)
        fig = []
        turn = not turn
    elif game.event <= 1:
        move_list = []
        qwe = GudBot.main(game.board, game.last_first_move, game.castle, game.kings, turn, 1)
        game.move(qwe)
        fig = []
        turn = not turn
        '''
